...
class User(db.Model):
    ...
    # when user deleted, post deleted, tag still remains 
    posts = db.relationship("Post", backref="user", cascade="all, delete-orphan")
...
class Post(db.Model):
    ...
    #testing below
    # currently working -when post is deleted, tag is still there
...
class Tag(db.Model):
    ...
    #testing below
    # currently working. When tag deleted, post still there. Write tests
...
class PostTag (db.Model):
    ...
    # tag_id = db.Column(db.Integer, db.ForeignKey("tags.id", ondelete = 'SET NULL'), primary_key=True, nullable = True)
    # will above work? We have a PK constraint below
...
#already working below with no error catching:
# from flask_sqlalchemy import SQLAlchemy
# from sqlalchemy.sql import func
